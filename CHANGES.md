...
### Major Changes
- **Removed Go implementation** - Migrated entirely to Rust
- **Workspace architecture** - Restructured into modular crates:
  - `kuiperdb-core` - Core library with all business logic
  - `kuiperdb-client` - HTTP client for REST API
  - `kuiperdb` - Main binary and server runtime
- **Dual usage modes**:
  - Embedded: Use kuiperdb-core or KuiperDb as library
  - REST API: Run KuiperDb binary, connect via kuiperdb-client

### Technical Details
- All core functionality (storage, indexing, search, embeddings, caching) in kuiperdb-core
- Clean separation between library and server runtime
- Shared workspace dependencies for consistency
- Preserved all existing features from v0.1.0
...
## Overview
Added comprehensive filtering capabilities to KuiperDb, allowing documents to be filtered by tags and metadata fields during searches.
